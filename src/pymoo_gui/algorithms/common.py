# Shared generation-payload, Pareto-front and callback utilities for GUI algorithms.

# ------------------------------------------------------------------------------------
# Module: common.py
# Summary: generation payload schema, Pareto-front extraction and optimization callback utilities.
# Implementation: population data is normalized, feasible nondominated fronts are selected and metrics are computed.
# Responsibility: supplies shared runtime data collection for algorithms in the doctoral-dissertation framework.
...
def known_pareto_front(problem: Any, diagnostics: Optional[list[str]] = None) -> Optional[np.ndarray]:
    ...
    # Args:
    # problem (Any): Problem instance that may expose `_gui_known_pf` or `pareto_front`.
    # diagnostics (Optional[list[str]]): Optional list collecting non-fatal issues.
    # Returns:
    # Optional[np.ndarray]: Finite objective matrix or `None` when unavailable.
    if problem is None:
        return None
...
def _feasible_nondominated_front_with_indices(
    ...
    # Args:
    # F (Optional[np.ndarray]): Objective matrix for the population.
    # CV (Optional[np.ndarray]): Constraint violation values aligned with `F`.
    # diagnostics (Optional[list[str]]): Optional diagnostic collector.
    # Returns:
    # tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[int]]:
    # Nondominated front, source row indices and number of selected rows.
    if F is None:
...
def make_generation_callback(
    ...
    # Args:
    # on_generation (Callable[[GenerationPayload], None]): Consumer called with each payload.
