"""
EN: Classic AGE-MOEA implementation integrated with the GUI registry.
"""

# ------------------------------------------------------------------------------------
# File: age.py
# Contents: AGE-MOEA survival, algorithm class and GUI factory definition.
# What happens here: AGE-MOEA estimates Pareto-front geometry and uses it for survival.
# Role in the framework: adds the classic adaptive-geometry AGE optimizer.
...
def _parse_positive_int(value: Any, field_name: str) -> int:
    """
    EN: Parse a positive integer AGE-MOEA configuration value.

    PL: Sprawdza, czy ustawienie algorytmu jest dodatnia liczba calkowita.
    """
    try:
        parsed = int(value)
...
def _parse_optional_positive_int(value: Any, field_name: str) -> Optional[int]:
    """
    EN: Parse an optional positive integer and preserve `None` for pymoo defaults.

    PL: Odczytuje dodatnia liczbe albo zostawia `None`, gdy ma dzialac ustawienie domyslne.
    """
    if value is None:
        return None
...
def point_2_line_distance(points: np.ndarray, start: np.ndarray, end: np.ndarray) -> np.ndarray:
    """
    EN: Compute squared distances from points to a line in objective space.

    PL: Liczy, jak daleko punkty sa od wskazanej prostej w przestrzeni celow.
    """
...
def find_corner_solutions(front: np.ndarray) -> np.ndarray:
    """
    EN: Return indices of extreme solutions used to normalize the current front.

    PL: Wybiera skrajne rozwiazania potrzebne do przeskalowania frontu.
    """
...
def normalize(front: np.ndarray, extreme: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """
    EN: Normalize a front using the hyperplane defined by its extreme points.

    PL: Skaluje front, aby cele byly porownywane w podobnym zakresie.
    """
    front = np.asarray(front, dtype=float)
    _, n_obj = front.shape
...
class AGEMOEASurvival(Survival):
    """
    EN: Environmental selection for classic AGE-MOEA.

    PL: Wybiera rozwiazania do kolejnej generacji, biorac pod uwage jakosc i ksztalt frontu.
    """

    def __init__(self) -> None:
        """
        EN: Initialize non-dominated sorting used by AGE-MOEA survival.

        PL: Przygotowuje sortowanie frontow Pareto uzywane przy selekcji.
        """
    ...
    def _do(self, problem, pop, *args, n_survive=None, **kwargs):
        """
        EN: Select survivors by rank and adaptive-geometry crowding score.

        PL: Zostawia najlepsze rozwiazania wedlug frontu Pareto i oceny roznorodnosci.
        """
    ...
    def survival_score(self, front: np.ndarray, ideal_point: np.ndarray) -> tuple[np.ndarray, float, np.ndarray]:
        """
        EN: Compute AGE-MOEA crowding values, estimated p-norm and normalization.

        PL: Liczy ocene rozwiazan, oszacowany ksztalt frontu i skale celow.
        """
    ...
    def compute_geometry(front: np.ndarray, extreme: np.ndarray, n_obj: int) -> float:
        """
        EN: Estimate the Minkowski p-norm that best matches the first front geometry.

        PL: Szacuje parametr opisujacy ksztalt aktualnego frontu rozwiazan.
        """
    ...
    def pairwise_distances(front: np.ndarray, p: float) -> np.ndarray:
        """
        EN: Compute pairwise Minkowski distances inside one front.

        PL: Liczy odleglosci miedzy rozwiazaniami na tym samym froncie.
        """
    ...
    def minkowski_distances(A: np.ndarray, B: np.ndarray, p: float) -> np.ndarray:
        """
        EN: Compute Minkowski distances between two objective matrices.

        PL: Liczy odleglosci miedzy dwoma grupami rozwiazan.
        """
...
class AGE(GeneticAlgorithm):
    """
    EN: Classic adaptive-geometry AGE-MOEA algorithm.

    PL: Algorytm AGE-MOEA, ktory dopasowuje selekcje do ksztaltu frontu Pareto.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        EN: Configure AGE-MOEA with pymoo-compatible genetic operators.

        PL: Ustawia rozmiar populacji oraz operatory tworzenia nowych rozwiazan.
        """
        super().__init__(
    ...
    def _set_optimum(self, **kwargs):
        """
        EN: Expose feasible rank-zero solutions as the current optimum.

        PL: Przekazuje do wyniku najlepszy aktualny front albo najmniej bledne rozwiazanie.
        """
...
def make_age(pop_size: int = 100, n_offsprings: Optional[int] = None) -> AGE:
    """
    EN: Create a configured classic AGE-MOEA algorithm for the GUI registry.

    PL: Tworzy gotowy algorytm AGE-MOEA z ustawieniami z formularza.
    """
    return AGE(
        pop_size=_parse_positive_int(pop_size, "pop_size"),
        n_offsprings=_parse_optional_positive_int(n_offsprings, "n_offsprings"),
